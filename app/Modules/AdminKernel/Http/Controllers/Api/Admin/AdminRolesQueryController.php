...
use Maatify\AdminKernel\Domain\List\AdminRolesCapabilities;
...
final readonly class AdminRolesQueryController
{
    public function __construct(
        private AdminRolesRepositoryInterface $reader,
    ...
    /**
     * ===============================
     * Admin Roles — QUERY (Assigned)
     * GET /admins/{id}/roles/query
     * ===============================
     *
     * - Read-only
     * - Assigned roles only
    ...
    public function __invoke(
        ...
        // 3) capabilities (Admin → Roles)
        $capabilities = AdminRolesCapabilities::define();

        // 4) resolve filters
        $resolved = $this->filters->resolve($query, $capabilities);

        // 5) execute reader
        $result = $this->reader->queryRolesForAdmin(
            $adminId,
            $query,
            $resolved
        );
