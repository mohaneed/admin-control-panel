...
readonly class PermissionsController
{
    public function __construct(
        private PermissionsReaderRepositoryInterface $reader,
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        $capabilities = PermissionsCapabilities::define();

        // 4) Resolve filters (INSTANCE call)
        $filters = $this->filterResolver->resolve($query, $capabilities);

        // 5) Execute reader
        $result = $this->reader->queryPermissions($query, $filters);
