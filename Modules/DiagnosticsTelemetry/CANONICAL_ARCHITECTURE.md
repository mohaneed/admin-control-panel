...
* **Semantic Coupling:**
  Consumers **MUST NOT** infer business meaning from telemetry fields.
...
* **Role:** Rule Enforcer.
* **Responsibility:**
  Normalize and validate inputs (actor types, severity, metadata size).
* **Invariants:**
...
4. **DTO Construction:**
    ...
    * Build `DiagnosticsTelemetryEventDTO` with `DiagnosticsTelemetryContextDTO`.
5. **Persistence (Infrastructure):**

    * Delegate to `DiagnosticsTelemetryLoggerInterface::write()`.
...
**Purpose:**
Human-oriented inspection of telemetry data via dashboards.

**Hard Constraints:**

* MUST live outside `app/Modules/DiagnosticsTelemetry`
...
3. **Throwing on Write**
   Diagnostics must never block the system.
4. **Audit Misuse**
   This module is NOT for compliance logging.
